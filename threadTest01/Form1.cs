...
namespace threadTest01
{
    public partial class Form1 : Form
    {
        ...
        public static bool CheckLRPairing = false;
        ...
        static public string dutCheckColorPri;
        static public string dutCheckColorSec;


        static public string dutFullBdAddressPri;
        static public string dutFullBdAddressSec;

        static public byte batLevel;
        ...
        static public string dutAncWriteSec;
        static public string dutBatLevel;
        static public string dutBatLevelPri;
        static public string dutBatLevelSec;
        static public string dutCheckPaired;
        ...
        static public int gBatPassLevel;
        ...
        private void clearFlag()
        {
            ...
            dutBatLevelPri = null;
            dutBatLevelSec = null;
            dutCheckColorPri = null;
            dutCheckColorSec = null;
            dutAncWritePri = null;
            dutAncWriteSec = null;            
            typeNg = null;
        }
        ...
        public static void handle(object s, USBInterface.ReportEventArgs a)
        {
            ...
            else if (a.Data[0] == 0x02 && a.Data[1] == 0x0b && a.Data[2] == USB_DEVICE_DATA_RSP_HS_SPP_RX) // SPP Test
            {
                ...
                {                    
                    ...
                    {
                        flag_check_LRPairing = false;
                        if(a.Data[10] == 0x05)
                        {
                            dutCheckPaired = "Paired";
                            CheckLRPairing = true;
                            Console.WriteLine("LR Paired");
                        }
                        else
                        {
                            dutCheckPaired = "Not Paired";
                            CheckLRPairing = false;
                            Console.WriteLine("LR paired is problem");
                        }
                    }
                    ...
                    else if (a.Data[9]== 0x0A && a.Data[10] == 0x01 && a.Data[11] == 0x00) // check color pri
                    {
                        ...
                        {                            
                            ...
                            {
                                dutFwColorPri = "BK";
                                dutCheckColorPri = dutFwColorPri;
                    ...
                    else if (a.Data[17] == 0x0A && a.Data[18] == 0x01 && a.Data[19] == 0x00) // check color sec
                    {
                        if (!CheckFwColorSec)
                        {
                            ...
                            {
                                dutFwColorSec = "BK";
                                dutCheckColorSec = dutFwColorSec;
...
#endif
                    else if (a.Data[5] == 0x5D && a.Data[8] == 0xD6 && a.Data[9] == 0x0C && a.Data[10] == 0x00) // check battery level
                    {
                        if (!CheckBatLevel)
                        {
                            batLevel = a.Data[12];
                            ...
                            dutBatLevel = batLevel.ToString();
                            //dutBatLevelPri = batLevelPri.ToString();
                            //dutBatLevelSec = batLevelSec.ToString();
...
#region config_relative
        private void loadConfig()
        {
            ...
            tempString = IniReader.IniReadValue("CONFIG", "BAT_PASS_LEVEL", pathConfigFile);
            gBatPassLevel = Convert.ToInt16(tempString);
            ...
            for(int j=0; j<gCountSequence; j++)
            {
                ...
                else if (gtestSequence[j] == "CHECK_BAT_LEVEL")
                {
                    string[] rowInfo = { j.ToString(), gtestSequence[j], gBatPassLevel.ToString()+" ↑", "-", "-" };
                    dataGridView1.Rows.Add(rowInfo);
                }
...
#region log_relative
        ...
        private void btnGetFWVersion_Click(object sender, EventArgs e)
        {
            ...
            byte a = 0;
            byte[] data = new byte[50];
            byte[] cmdData = new byte[] { 0x05, 0x5A, 0x06, 0x00, 0x00, 0x0A, 0x01, 0xF5, 0x01, 0x00 }; // CC500BT2 function
        ...
        private void btnGetBDAddr_Click(object sender, EventArgs e)
        {
            ...
            byte a = 0;
            byte[] data = new byte[50];            
            byte[] cmdData = new byte[] { 0x05, 0x5A, 0x03, 0x00, 0xD5, 0x0C, 0x00 }; // Airoha default
        ...
        private void btnGetFWMode_Click(object sender, EventArgs e)
        {
            ...
            byte a = 0;
            byte[] data = new byte[50];
            byte[] cmdData = new byte[] { 0x05, 0x5A, 0x06, 0x00, 0x00, 0x0A, 0x00, 0xF5, 0x01, 0x00 }; // CC500BT2 Function
        ...
        private void btnSetFactorymode_Click(object sender, EventArgs e)
        {
            ...
            byte a = 0;
            byte[] data = new byte[50];
            byte[] cmdData = new byte[] { 0x05, 0x5A, 0x05, 0x00, 0x01, 0x0A, 0x00, 0xF5, 0x01 };
        ...
        private void btnSetIGDownloadmode_Click(object sender, EventArgs e)
        {
            dev.StartAsyncRead();
            byte a = 0;
            byte[] data = new byte[50];
            byte[] cmdData = new byte[] { 0x05, 0x5A, 0x05, 0x00, 0x01, 0x0A, 0x00, 0xF5, 0x02 };
        ...
        private void btnEnterDUTMode_Click(object sender, EventArgs e)
        {
            ...
            byte a = 0;
            byte[] data = new byte[50];
            byte[] cmdData = new byte[] { 0x05, 0x5A, 0x04, 0x00, 0x00, 0x01, 0x01, 0x00};
        ...
        private void btnGetBatlevel_Click(object sender, EventArgs e)
        {
            dev.StartAsyncRead();
            byte a = 0;
            byte[] data = new byte[50];
        ...
        private void btnToneGenerator_Click(object sender, EventArgs e)
        {

        }

        private void btnPairingmode_Click(object sender, EventArgs e)
        {
    ...
    class backWork2
    {
...
#if false // disconnect -> factory reset (for ATH-CKS50TW2, when send disconnect command, earbuds enter strange state)
        ...
        private bool checkBatLevel(byte batlevel)
        {            
            if (batlevel < Form1.gBatPassLevel) { return false; }
            else { return true; }
        }
...
END
        ...
        private bool procTestCheckBatLevel(int index)
        {
            ...
            {
                tempBat = Form1.batLevel;
                //tempBatPri = Form1.batLevelPri;
                //tempBatSec = Form1.batLevelSec;

                // set test value (nothing)
                mainForm.gTestValue[index] = Convert.ToString(Form1.batLevel);

                if (!checkBatLevel(tempBat) ){ flag_ng = 1; }
            }
        ...
        {            
            ...
            try
            {
                ...
                byte a = 0;
                byte[] data = new byte[50];
                byte[] cmdData = new byte[] { 0x05, 0x5A, 0x06, 0x00, 0x00, 0x0A, 0x01, 0xF5, 0x01, 0x00 };
